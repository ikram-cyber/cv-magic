export const Magic = {
    ...
    // Mesin AI Penulis Ringkasan
    generateSummary(data) {
        const r = (data.role || '').toLowerCase();
        const lang = data.lang || 'id';
        // Deteksi apakah pelamar punya pengalaman atau kosong (Fresh Grad)
        const hasExp = data.experiences && data.experiences.some(e => e.title || e.company);

        if(lang === 'id') {
            if (!hasExp) {
                // MODE FRESH GRADUATE (Belum ada pengalaman)
            ...
            // MODE BERPENGALAMAN (Sudah ada pengalaman)
    ...
    generateCoverLetter(data) {
        ...
        // Bahasa Surat Menyesuaikan Pengalaman
