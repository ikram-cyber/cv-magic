...
export const FormEditor = {
    ...
    renderExp() {
        ...
        State.data.experiences.forEach((exp, index) => {
            ...
            div.innerHTML = `
                <div class="absolute top-2 right-2 flex gap-3 text-sm">
                    ${index > 0 ? `<i class="fas fa-arrow-up text-cyan-500 cursor-pointer hover:text-cyan-300 transition" onclick="window.moveExp(${index}, -1)"></i>` : ''}
                    ${index < State.data.experiences.length - 1 ? `<i class="fas fa-arrow-down text-cyan-500 cursor-pointer hover:text-cyan-300 transition" onclick="window.moveExp(${index}, 1)"></i>` : ''}
                    <i class="fas fa-trash text-rose-500 cursor-pointer hover:text-rose-300 transition ml-2" onclick="window.deleteExp(${index})"></i>
                </div>
        ...
        // PERBAIKAN LOGIKA GESER & HAPUS (Menggunakan FormEditor.renderExp)
