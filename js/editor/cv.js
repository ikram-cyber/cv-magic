class CVBuilder {
    constructor() {
        this.data = { font: 'font-sans', photo: null, sig: null, exps: [], edus: [] };
        this.init();
    }
    ...
    bindTabs() {
        ...
        document.getElementById('btn-export-cv').onclick = async () => {
            ...
            await html2pdf().set({margin: 0, filename: 'My_CV.pdf', image: { type: 'jpeg', quality: 1 }, html2canvas: { scale: 2 }, jsPDF: { unit: 'mm', format: 'a4', orientation: 'portrait' }}).from(document.getElementById('cv-paper')).save();
    ...
    bindMedia() {
        ...
        document.getElementById('btn-save-sig').onclick = () => {
            ...
            localStorage.setItem('cv-sig', this.data.sig);
    ...
    renderLists() {
        ...
        this.data.exps.forEach((x,i) => {
            elExp.innerHTML += `<div class="bg-slate-900 p-2 rounded flex gap-2"><input class="flex-1 bg-transparent text-xs outline-none" placeholder="Posisi" value="${x.role}" oninput="appCV.upD('exps',${i},'role',this.value)"><input class="flex-1 bg-transparent text-xs outline-none border-l border-slate-700 pl-2" placeholder="PT" value="${x.comp}" oninput="appCV.upD('exps',${i},'comp',this.value)"><input class="w-16 bg-transparent text-xs outline-none border-l border-slate-700 pl-2" placeholder="Tahun" value="${x.year}" oninput="appCV.upD('exps',${i},'year',this.value)"></div>`;
        });
        const elEdu = document.getElementById('cv-edu-list'); elEdu.innerHTML = '';
        this.data.edus.forEach((x,i) => {
            elEdu.innerHTML += `<div class="bg-slate-900 p-2 rounded flex gap-2"><input class="flex-1 bg-transparent text-xs outline-none" placeholder="Sekolah" value="${x.school}" oninput="appCV.upD('edus',${i},'school',this.value)"><input class="flex-1 bg-transparent text-xs outline-none border-l border-slate-700 pl-2" placeholder="Jurusan" value="${x.degree}" oninput="appCV.upD('edus',${i},'degree',this.value)"><input class="w-16 bg-transparent text-xs outline-none border-l border-slate-700 pl-2" placeholder="Tahun" value="${x.year}" oninput="appCV.upD('edus',${i},'year',this.value)"></div>`;
        });
    }
    ...
    renderPaper() {
        ...
        p.className = `a4-sheet p-[15mm] ${this.data.font}`;
        p.innerHTML = `
            <div class="flex gap-5 border-b-2 border-sky-600 pb-4 mb-4">
                <div class="w-[30mm] h-[40mm] bg-slate-100 border-2 border-slate-900 rounded overflow-hidden flex justify-center items-center">
                ...
                <div class="flex-1">
                    ...
                    <h2 class="text-xs font-bold text-sky-600 uppercase tracking-widest mt-1 mb-3">${d.t}</h2>
            ...
            <div class="space-y-4">
                <div><h3 class="text-xs font-black uppercase border-b-2 border-slate-200 mb-1">Profil</h3><p class="text-[10px] leading-relaxed text-justify">${d.prof}</p></div>
                <div><h3 class="text-xs font-black uppercase border-b-2 border-slate-200 mb-1">Pengalaman Kerja</h3><div class="space-y-2">${this.data.exps.map(x=>`<div class="flex justify-between"><div class="flex-1"><p class="text-[11px] font-bold">${x.role||'Posisi'}</p><p class="text-[10px] text-sky-600 font-bold">${x.comp||'Perusahaan'}</p></div><div class="text-[10px] font-bold text-slate-500">${x.year||'Tahun'}</div></div>`).join('')}</div></div>
                <div><h3 class="text-xs font-black uppercase border-b-2 border-slate-200 mb-1">Pendidikan</h3><div class="space-y-2">${this.data.edus.map(x=>`<div class="flex justify-between"><div class="flex-1"><p class="text-[11px] font-bold">${x.school||'Sekolah/Kampus'}</p><p class="text-[10px] text-sky-600 font-bold">${x.degree||'Jurusan'}</p></div><div class="text-[10px] font-bold text-slate-500">${x.year||'Tahun'}</div></div>`).join('')}</div></div>
            </div>
            <div class="absolute bottom-10 right-10 text-center w-32">
                ...
                <p class="text-[10px] font-black border-t border-slate-900 uppercase pt-1">${d.n}</p>
            </div>
        `;
    }
}
